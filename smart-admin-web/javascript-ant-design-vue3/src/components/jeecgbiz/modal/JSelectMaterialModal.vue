    ...
    <a-row :gutter="10" style="padding: 10px; margin: -10px">
      <a-col :md="24" :sm="24">
        ...
        <div class="table-page-search-wrapper">
          ...
          >
            <template v-slot:customBarCode="text, record">
            ...
            <template v-slot:customName="text, record">
              {{ record.name }}
...
<script>
  ...
  export default {
    ...
    components: {
      MaterialModal: () => import('/@/views/business/material/modules/MaterialModal.vue'),
    },
    ...
    methods: {
      ...
      handleChangeOtherField() {
        let mpStr = getMpListShort(useMaterialStore().getPropertyList);
        if (mpStr) {
      ...
      getQueryParams() {
        ...
        param.mpList = getMpListShort(useMaterialStore().getPropertyList); //扩展属性
      ...
      handleSubmit() {
        ...
        $emit(that, 'ok', that.selectMaterialRows, that.selectMaterialIds);
        that.searchReset(0);
        that.close();
      },
      //获取选择信息
