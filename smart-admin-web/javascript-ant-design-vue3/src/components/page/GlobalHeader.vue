...
export default {
  ...
  mounted() {
    ...
    if (
      window.location.host === 'cloud.huaxiaerp.vip' ||
      window.location.host === 'cloud.huaxiaerp.com'
    ) {
      this.showJump()
    }
  },
  ...
  methods: {
    ...
    searchGlobalHeader(key, id, title, component) {
      $emit(this, 'searchGlobalLayout', key, id, title, component)
    },
    initSystemConfig() {
  ...
  emits: ['searchGlobalLayout', 'toggle'],
}
</script>
