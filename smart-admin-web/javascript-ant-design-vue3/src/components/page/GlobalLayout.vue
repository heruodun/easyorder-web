<template>
  <a-layout class="layout" :class="[device]">
    ...
    <a-layout
      ...
      :style="{
        paddingLeft:
          fixSiderbar && isDesktop() ? `${sidebarOpened ? 150 : 80}px` : '0',
      }"
    >
      <!-- layout header -->
      <global-header
      ...
      <!-- layout content -->
      <a-layout-content
        :style="{ height: '100%', paddingTop: fixedHeader ? '49px' : '0' }"
      >
        <slot></slot>
      </a-layout-content>
...
export default {
  ...
  methods: {
    ...
    searchGlobalLayout(key, id, title, component) {
      $emit(this, 'dynamicRouterShow', key, id, title, component)
    },
    //update-end-author:taoyan date:20190430 for:动态路由title显示配置的菜单title而不是其对应路由的title
  },
  emits: ['dynamicRouterShow'],
}
</script>
