<template>
  <div class="j-super-query-box">
    <slot name="button" :isActive="superQueryFlag" :isMobile="izMobile" :open="handleOpen" :reset="handleReset">
      <a-tooltip v-bind="tooltipProps" v-if="superQueryFlag" :mouseLeaveDelay="0.2">
        ...
        <a-row>
              ...
              <a-row type="flex" style="margin-bottom: 10px" :gutter="16" v-for="(item, index) in queryParamsModel" :key="index">
                ...
                <a-col :md="8" :xs="24" style="margin-bottom: 12px">
                  <template v-if="item.dictCode">
                    ...
                    <template v-else>
                      <j-multi-select-tag v-show="allowMultiple(item)" v-model:value="item.val" :dictCode="item.dictCode" placeholder="请选择" />
                      <j-dict-select-tag v-show="!allowMultiple(item)" v-model:value="item.val" :dictCode="item.dictCode" placeholder="请选择" />
                    </template>
                  </template>
                  <j-popup
                    v-bind="item.popup"
                  ...
                  />
                  <j-area-linkage v-model:value="item.val" v-else-if="item.type === 'area-linkage' || item.type === 'pca'" style="width: 100%" />
          ...
          <a-col :sm="24" :md="5">
            ...
            <a-card class="j-super-query-history-card" :bordered="true">
              ...
              <a-tree v-else class="j-super-query-history-tree" showIcon :treeData="saveTreeData" :selectedKeys="[]" @select="handleTreeSelect">
              </a-tree>
            </a-card>
          </a-col>
        </a-row>
    ...
    * */
    ...
    methods: {
      ...
      emitCallback(event = {}) {
        ...
        $emit(this, this.callback, params, matchType);
      },
      handleCancel() {
        this.close();
      },
