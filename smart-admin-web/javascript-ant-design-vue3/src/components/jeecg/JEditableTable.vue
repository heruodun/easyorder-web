          ...
          >
            ...
            <template v-for="col in columns" :key="col.key">
              <div class="td" v-show="col.type !== formTypes.hidden" :style="buildTdStyle(col)">
                <span v-show="col.type === formTypes.inputNumber" style="padding: 0 2px">{{ statisticsColumns[col.key] }}</span>
              </div>
            </template>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      removeRows(id) {
        ...
        this.rows = rows;
        $emit(this, 'deleted', this.getDeleteIds(), this);
        this.$nextTick(() => {
          // 更新formValues
          this.updateFormValues();
          // 重新计算统计
      ...
      emitDragged(oldIndex, newIndex) {
        $emit(this, 'dragged', { oldIndex, newIndex, target: this });
      },
      ...
      elemValueChange(type, rowSource, columnSource, value) {
        ...
        $emit(this, 'valueChange', { type, row, column, value, target: this });
      },
    ...
    emits: ['selectRowChange', 'added', 'inserted', 'deleted', 'dragged', 'valueChange'],
  };
</script>
