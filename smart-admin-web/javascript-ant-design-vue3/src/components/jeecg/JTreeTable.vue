<template>
  ...
  >
    ...
    >
      <slot v-bind="{ text, record, index }" :name="slotItem"></slot>
    </template>
  </a-table>
</template>
...
export default {
    ...
    loadData(id = this.topValue, first = true, url = this.url) {
      ...
      return getAction(url, params)
        .then((res) => {
          ...
          $emit(this, 'requestSuccess', { first, dataSource, res })
          return Promise.resolve(dataSource)
        })
        .finally(() => $emit(this, 'requestFinally', { first }))
    },
    ...
    handleExpand(expanded, record) {
      ...
      if (expanded) {
        ...
        if (record.children[0].isLoading === true) {
          this.loadData(record.id, false, this.getChildrenUrl).then(
            (dataSource) => {
  ...
  emits: ['requestBefore', 'requestSuccess', 'requestFinally'],
}
</script>
